...
## 方式〇：本地写作编辑器（本机，带界面，推荐日常用）

本机运行 `python editor.py`，打开 http://127.0.0.1:8010 即可在浏览器里写作：
输入 Markdown → 右侧实时预览（与发布渲染一致）→ 「保存并发布」自动完成
写文件、重建站点、git 提交并推送。详见 `content/notes/using-editor.md`。

## 方式一：纯浏览器添加（不装本地工具时用，配了自动构建）
